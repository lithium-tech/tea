...
arrow::Status CopyDirectoryToS3(const std::filesystem::path& local_dir,
  ...
  for (const auto& entry : std::filesystem::recursive_directory_iterator(local_dir)) {
    if (entry.is_regular_file()) {
      ...
      std::string content((std::istreambuf_iterator<char>(local_file)),
...
TEST_F(DeletionVectorTest, SimpleScanAndMetrics) {
  ...
  std::string metadata_location = "s3a://" + s3_base_path + "/metadata/00004-ae0294d0-1de5-4fab-97f1-cd80e0078fa4.metadata.json";
  ...
  ASSIGN_OR_FAIL(auto defer, table_creator->CreateTable(
      {GreenplumColumnInfo{.name = "c1", .type = "int4"},
