...
std::pair<iceberg::ice_tea::ScanMetadata, PlannerStats> FromIcebergWithLocation(
  ...
  auto result = [&]() -> arrow::Result<iceberg::ice_tea::ScanMetadata> {
    ...
    if (!schema) {
      ...
      std::optional<std::string> maybe_manifest_list_path = tea::GetManifestListPathForSnapshot(table_metadata, snapshot_id);
      ...
      entries_stream = iceberg::ice_tea::AllEntriesStream::Make(
          fs, maybe_manifest_list_path.value(), use_reader_schema, table_metadata->partition_specs,
          schema, partition_pruning_stats_filter,
      ...
      if (filter) {
        entries_stream = std::make_shared<FilteringEntriesStream>(
            entries_stream, filter, schema, timestamp_to_timestamptz_shift_us);
      }
    }
